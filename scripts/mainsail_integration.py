"""Prepare idempotent RatOS/Mainsail navigation and Nginx files."""
import argparse
import json
from pathlib import Path


MARKER_START = "    # AUTOPA MANAGED START"
MARKER_END = "    # AUTOPA MANAGED END"
MANAGED_BLOCK = (
    MARKER_START + "\n"
...
def merge_navigation(navigation, entry):
    ...
    if not isinstance(entry, dict) or entry.get("title") != "AutoPA":
        raise ValueError("AutoPA navigation entry is invalid")
    filtered = [
        item for item in navigation
        if not (
            isinstance(item, dict)
            and (item.get("title") == "AutoPA"
                 or item.get("href") == "/autopa/"))
    ]
