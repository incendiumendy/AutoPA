...
NAVIGATION=${AUTOPA_MAINSAIL_NAVIGATION:-"$HOME/printer_data/config/.theme/navi.json"}
NAVIGATION_ENTRY="$PROJECT_DIR/config/mainsail-autopa-navigation.json"
NGINX_SITE=${AUTOPA_NGINX_SITE:-"/etc/nginx/sites-available/mainsail"}
NGINX_LOCATION="$PROJECT_DIR/config/nginx-autopa-location.conf"
NGINX_SNIPPET="/etc/nginx/snippets/autopa.conf"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
BACKUP_DIR="$HOME/printer_data/config/.autopa-backup/$STAMP-mainsail"
...
for path in "$NAVIGATION" "$NAVIGATION_ENTRY" "$NGINX_SITE" \
        "$NGINX_LOCATION"; do
    [ -f "$path" ] || {
        printf '%s\n' "Required file not found: $path" >&2
        exit 1
    }
done
...
printf '%s\n' "AutoPA added to the Mainsail navigation."
printf '%s\n' "Dashboard path: /autopa/"
